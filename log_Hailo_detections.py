...
def center_crop_square(image):
    """Center crop image to square maintaining aspect ratio."""
...
def extract_detections(hailo_output, class_names, valid_classes, threshold=0.5):
...
def main():
    ...
    with Hailo(args.model) as hailo:
        ...
        with Picamera2() as picam2:
            ...
            if video_h > video_w:
                lores_h = int(round(model_h * (video_h/video_w)))
                lores = {'size': (model_w, lores_h), 'format': 'RGB888'}
            else:
            ...
            try:
                while True:
                    ...
                    detections = extract_detections(results, class_names, valid_classes, args.confidence)
                    
                    if detections:
                        # Generate filename with timestamp
                        timestamp = time.strftime("%Y%m%d-%H%M%S")
                        filename = f"hailo-{timestamp}.jpg"
                        filename_lores = f"hailo-{timestamp}_lores.jpg"
                        ...
                        lores_path = os.path.join(image_detections_path, filename_lores)
